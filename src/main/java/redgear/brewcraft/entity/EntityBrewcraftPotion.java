...
public class EntityBrewcraftPotion extends EntityPotion {
	...
	protected void onImpact(MovingObjectPosition movingobjectposition) {
		 ...
		 * double d7;
		 * 
		 * s = "iconcrack_" + Brewcraft.potions.itemID + "_" +
		 * getPotionStack().getItemDamage();
		 * 
		 * for (j1 = 0; j1 < 8; ++j1)
		 * worldObj.spawnParticle(s, posX, posY, posZ, random.nextGaussian() *
		 * 0.15D, random.nextDouble() * 0.2D,
		 * random.nextGaussian() * 0.15D);
		 * 
		 * if (worldObj.isRemote) { //TODO: Make particles work right on sever.
		 * It requires packets and stuff that I just don't feel like doing right
		 * now - Blackhole
		 * 
		 ...
		 * 
		 * EntityFX entityfx =
		 * Minecraft.getMinecraft().renderGlobal.doSpawnParticle(s1, posX + d4 *
		 * 0.1D,
		 * posY + 0.3D, posZ + d6 * 0.1D, d4, d5, d6);
		 * //new EntitySpellParticleFX(worldObj, posX + d4 * 0.1D, posY + 0.3D,
		 * posZ + d6 * 0.1D, d4, d5, d6);
		 * 
		 * 
		 * if (isInstant)
		 ...
		 * }
		 * }
		 * worldObj.playSoundEffect(posX, posY, posZ, "random.glass", 1.0F,
