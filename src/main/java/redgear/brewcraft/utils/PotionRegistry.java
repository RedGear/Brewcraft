...
public class PotionRegistry implements redgear.brewcraft.api.PotionAPI.IPotionRegistry{
	...
	public void addPotion(String name, Potion effect, int strength, int duration, boolean hasDescription) {
		if(effect !=null) {
			SimpleItem bottle = PotionPlugin.potions.addMetaItem(new SubItemPotion("bottle" + name, false, effect, duration, strength, hasDescription));
			SimpleItem splash = PotionPlugin.potions.addMetaItem(new SubItemPotion("splash" + name, true, effect, duration, strength, hasDescription));
			Fluid potion = FluidUtil.createFluid(new FluidPotion("potion" + name, effect.getLiquidColor(), bottle, splash), PotionPlugin.potionTexture);
			
			FluidContainerRegistry.registerFluidContainer(potion, bottle.getStack(), PotionPlugin.emptyBottle.getStack());
			FluidContainerRegistry.registerFluidContainer(potion, splash.getStack(), IngredientPlugin.splashBottle.getStack());
		}
		else {
