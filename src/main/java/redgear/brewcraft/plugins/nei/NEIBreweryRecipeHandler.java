...
public class NEIBreweryRecipeHandler extends TemplateRecipeHandler {
	...
	/*
	 * public List<String> provideItemTooltip(GuiRecipe guiRecipe, ItemStack
	 * itemStack, List<String> currenttip, CachedBaseRecipe crecipe, Point relMouse)
	 * { for (PositionedStack stack : crecipe.getIngredients()) { if (stack
	 * instanceof PositionedStackAdv && ((PositionedStackAdv)
	 * stack).getRect().contains(relMouse)) { currenttip = ((PositionedStackAdv)
	 * stack).handleTooltip(guiRecipe, currenttip); } } for (PositionedStack stack :
	 * crecipe.getOtherStacks()) { if (stack instanceof PositionedStackAdv &&
	 * ((PositionedStackAdv) stack).getRect().contains(relMouse)) { currenttip =
	 * ((PositionedStackAdv) stack).handleTooltip(guiRecipe, currenttip); } }
	 * PositionedStack stack = crecipe.getResult(); if (stack instanceof
	 * PositionedStackAdv && ((PositionedStackAdv)
	 * stack).getRect().contains(relMouse)) { currenttip = ((PositionedStackAdv)
	 * stack).handleTooltip(guiRecipe, currenttip); } return currenttip; }
	 * 
	 */
}
