...
public class BrewcraftTickHandler implements ITickHandler {
	...
	public void tickEnd(EnumSet<TickType> type, Object... tickData) {
		...
		if (player.isPotionActive(Brewcraft.creeper)) {
			...
			if (player.isPotionActive(Brewcraft.creeper)
					&& player.getActivePotionEffect(Brewcraft.creeper).getDuration() == 8)
				player.worldObj.playSoundAtEntity(player, "mob.creeper.say", 1F, 1F);

			if (player.isPotionActive(Brewcraft.creeper)
					&& player.getActivePotionEffect(Brewcraft.creeper).getDuration() == 4)
				player.worldObj.playSoundAtEntity(player, "mob.creeper.say", 1F, 1F);

			if (player.isPotionActive(Brewcraft.creeper)
					&& player.getActivePotionEffect(Brewcraft.creeper).getDuration() == 1
		...
		if (player.isPotionActive(Brewcraft.angel))
			if (player.worldObj.rand.nextDouble() > 0.8D) {
				player.heal(0.5F);
		...
		if (player.isPotionActive(Brewcraft.flight) && player.getActivePotionEffect(Brewcraft.flight).getDuration() > 1
				&& player.capabilities.isCreativeMode == false) {
			player.capabilities.allowFlying = true;
			if (player.isPotionActive(Brewcraft.flight)
					&& player.getActivePotionEffect(Brewcraft.flight).getDuration() == 1
					|| player.isPotionActive(Brewcraft.flight)
					&& player.getActivePotionEffect(Brewcraft.flight).getDuration() < 1) {
